...
const CLOUDINARY_URL =
  "https://api.cloudinary.com/v1_1/product243/image/upload";
const CLOUDINARY_UPLOAD_PRESET = "Display0505";

const Screen2 = ({ navigation, route }) => {
  const category = route.params.category;

  const CurrentUserId = getAuth();
  const userUID = CurrentUserId.currentUser.uid;
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      ...
      if (selectedImages.length + newImages.length > MAX_IMAGES) {
        return Alert.alert(
          "Limit Exceeded",
          "You can select up to 4 images only."
        );
      }
  ...
  const handlePress = async () => {
    ...
    if (uploadedImageUrls) {
      ...
      uploadDataToServer(uploadedImageUrls);
      setLoading(false);
  ...
  const uploadDataToServer = async (uploadedImageUrls) => {
    // console.log(uploadedImagseUrls);
    try {
      console.log("Server called...");
      await axios
        .post(`${BASE_URL}/post/create/${userUID}`, {
          category: category,
        ...
        .then((response) => {
          console.log("uplodaing data to serve.....");
      ...
      <View style={styles.innerContainer}>
        ...
        <ScrollView style={styles.scrollContainer}>
          ...
          <View style={styles.imageContainer}>
            <Text
              style={[styles.label, { paddingStart: 20, paddingBottom: 16 }]}
            >
              Upload Images
            </Text>
          ...
          <Modal visible={loading || uploadComplete} transparent>
            <View style={styles.modalOverlay}>
              <View style={styles.modalContent}>
                {loading ? (
                  <ActivityIndicator size="60" color={Color.secondary} />
                ) : (
                  <LottieView
